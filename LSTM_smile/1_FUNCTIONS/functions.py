...
def list_invers_scaled(scaled_list, max_value_list, min_value = 0):
    invers_scaled_list= list()
    for i in  range(len(scaled_list)):
        output_array = (scaled_list[i]+ min_value )* max_value_list[i]
        invers_scaled_list.append(output_array)
    return invers_scaled_list

    
def list_to_mms(input_list):
    mms_list=[]
    scaler = MinMaxScaler(feature_range = (0, 1))
    for elem in input_list:
        training_scaled = scaler.fit_transform(elem)

        mms_list.append(np.nan_to_num(training_scaled))
    return  mms_list, scaler
